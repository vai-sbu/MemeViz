...
MemeViz::MemeViz(QWidget *parent)
	: QMainWindow(parent)
{
	write_text_to_log_file("MemeVizStart\n");
	ui.setupUi(this);
	...
	else
	{
		...
		if (reply->error() == QNetworkReply::NoError) 
		{
			...
			if (tmpImg.loadFromData(reply->readAll()))
			{
				...
				scene_meme->setSceneRect(0, 0, imgW, imgH);
				loaded_img = true;	
...
void MemeViz::on_pushButton_pieArc_clicked()
{
...
void MemeViz::on_horizontalSlider_pieAng_sliderReleased()
{
...
double MemeViz::computePieAngles()
{
	cv::Mat src2;
	int thresh = 100;
	int max_thresh = 255;
...
double MemeViz::computePieError() {
	return 1.5;
}

void MemeViz::generateNeighborAngles() {
...
double MemeViz::acceptance_probability(double old_cost, double new_cost, double T) {
...
void MemeViz::on_pushButton_optimizePie_clicked() {		
...
void MemeViz::on_doubleSpinBox_yMaxBarFill_valueChanged()
{
...
void MemeViz::on_pushButton_barArea_clicked()
{
...
void MemeViz::optimizeBar() {
	double old_cost = computePieAngles();
	double T = 1.0;
	double T_min = 0.00001;
	double alpha = 0.9;
	std::vector<double> curr_hts = angles;
	while (T > T_min) {
		int i = 1;
		int j = 1;
		while (i <= 200 & j <= 200) {
...
{
	write_text_to_log_file("MemeVizClose\n");
	QApplication::quit();
}
...
//File Logging
void MemeViz::write_text_to_log_file(const std::string &text)
{
	std::ofstream log_file("meme_viz_log_file_DC.txt", std::ios_base::out | std::ios_base::app);
	log_file << text;
}